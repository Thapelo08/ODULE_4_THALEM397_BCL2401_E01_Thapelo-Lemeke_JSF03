    ...
    <div v-else class="container flex justify-center items-center p-6 min-h-screen m-auto">
      <div v-if="product" class="bg-white p-6 rounded shadow-lg mt-5">
        ...
        <div class="flex justify-center items-center mt-5">
          <img :src="product.image" :alt="product.title" class="w-96 h-64 object-cover mb-4 rounded" />
        </div>
...
export default {
  ...
  props: {
    params: {
      type: Object,
      required: true,
      validator: value => typeof value.id === 'number' || typeof value.id === 'string'
    }
  },
  setup(props) {
...
<style scoped>
/* Custom width and height for the product image */
.custom-image-width {
  width: 400px; /* Adjust as needed */
  height: 256px; /* Adjust as needed */
}

/* Custom styling for buttons (if needed) */
.custom-button {
  box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); /* Custom shadow for buttons */
}

/* Custom focus ring (if needed) */
.custom-focus-ring {
  box-shadow: 0 0 0 3px rgba(255, 105, 180, 0.4); /* Custom focus ring color */
}
</style>
